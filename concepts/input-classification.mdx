...
description: "Learn how GitResolve's classifyInput function categorizes every input into one of seven types before processing begins."
---
...
<Note>
`resume_url` is defined as a distinct `InputType` in the type system but is **not** auto-detected by `classifyInput`. A URL ending in `.pdf` hosted on a non-git domain resolves to `portfolio` through the current classification logic. Use `resume_url` when you are constructing a `ResolverResult` manually or extending the resolver.
</Note>

## Classification decision flow

`classifyInput` applies rules in strict order and returns as soon as a match is found. Passing through all seven steps without a match is only possible for the `unknown` case.
...
| `resume_url` | Treated as a remote resume document; the response body is processed similarly to `resume_file`. |
