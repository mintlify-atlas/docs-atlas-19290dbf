---
title: "Puppeteer Provider — Headless Chromium Rendering | GitResolve"
...
For a **global CLI install** the bundled Puppeteer may not be on the path. If `gitresolve --provider puppeteer` reports that the provider is unavailable, install Puppeteer globally alongside it:
...
<Tip>
  Set `BROWSER_PROVIDER=puppeteer` in your shell profile to make Puppeteer the default without passing `--provider` every time.
