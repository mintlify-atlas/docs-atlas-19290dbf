...
<ResponseField name="'gist'" type="GitLinkType">
  A GitHub Gist. This value is defined in the type union for completeness; `parseGitLink` currently returns `null` for `gist.github.com` URLs because that hostname is not in `GIT_HOSTS`.
</ResponseField>
