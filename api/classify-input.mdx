...
Classifies a raw URL from any of the six recognised git provider hostnames into a typed `ExtractedGitLink`. Unlike `parseRepoUrl`, this function also handles profile pages, gists, PRs, issues, and the `www.` subdomain variants.
...
<ResponseField name="url" type="string">
  For repo links this is the `normalized` canonical URL from `parseRepoUrl`. For PR/issue links this is the original `rawUrl` to preserve the exact contribution reference. For profile and gist links this is `rawUrl` as-is.
</ResponseField>
<ResponseField name="provider" type="'github' | 'gitlab' | 'bitbucket'">
...
<ResponseField name="username" type="string">
  The extracted owner username (first path segment, or second segment for gists on `gist.github.com`).
</ResponseField>
...
|---|---|
| `"gist"` | Host is `gist.github.com` or first path segment is `gist` |
...
// Gist
parseGitLink('https://gist.github.com/sindresorhus/abc123');
// { url: '...', provider: 'github', type: 'gist', username: 'sindresorhus' }
