...
GitResolve supports three strategies for fetching page content. Puppeteer is bundled as a dependency and used by default when available — you only need to intervene when you want to force a specific provider or use Browserless.

| Provider | How to Enable | JS Rendering | Best For |
|---|---|---|---|
| `puppeteer` | Bundled — available by default | ✅ Full | SPAs, JavaScript-heavy portfolio sites |
...
See the [Browser Providers guide](/providers/overview) for Browserless Docker setup and provider configuration details.

## Enterprise ATS Integrations

<Info>
  Are you processing large CSV exports from Lever, Greenhouse, or Ashby with custom column mappings? Native zero-configuration ATS integrations are being prioritized based on user demand. [Open an issue](https://github.com/ClyrisAI/gitresolve/issues) to register your platform and help shape the roadmap.
</Info>
